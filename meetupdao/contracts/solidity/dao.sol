...
contract BlockchainDevs is owned, tokenRecipient {
    ...
    function becomeMember(string memberName) public {
        ...
        members[id] = Member({member: msg.sender, memberSince: joiningTime, name: memberName});
    ...
    function newProposal(
        ...
        uint minutesForVoting,
        bytes transactionBytecode
    )
        onlyMembers public
        returns (uint proposalID)
    {
        if (minutesForVoting == 0) {
           minutesForVoting = debatingPeriodInMinutes;
        }
        ...
        p.votingDeadline = now + minutesForVoting * 1 minutes;
    ...
    function newProposalInEther(
        ...
        returns (uint proposalID)
    {
        return newProposal(beneficiary, etherAmount * 1 ether, jobDescription, uploadFileHash, minutesForVoting, transactionBytecode);
    }
        ...
        returns (uint voteID)
    {
        ...
        require(now < p.votingDeadline);                // If time has left before voting deadline
